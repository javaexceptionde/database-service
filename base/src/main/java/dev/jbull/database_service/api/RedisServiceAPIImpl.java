...
import dev.jbull.database_service.redis.RedisSessionImpl;

import java.util.HashMap;
import java.util.Map;

public class RedisServiceAPIImpl implements RedisServiceAPI{
    private final Map<String, RedisSession> sessionMap = new HashMap<>();

    @Override
    public RedisSession startSession(String session) {
        if (sessionMap.containsKey(session)){
            throw new SessionException("The session is already in use");
        }else {
            sessionMap.put(session, new RedisSessionImpl(session));
            return sessionMap.get(session);
        }
    }

    @Override
    public RedisSession getSession(String session) {
        if (sessionMap.containsKey(session)){
            return sessionMap.get(session);
        }
